...
class CreatePatientFlagsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('patientFlags', function (Blueprint $table) {
            $table->id();
            $table->bigInteger('patientId')->unsigned();
            $table->foreign('patientId')->references('id')->on('patients')->onUpdate('cascade')->onDelete('cascade');
            $table->bigInteger('flagId')->unsigned();
            $table->foreign('flagId')->references('id')->on('flags')->onUpdate('cascade')->onDelete('cascade');
