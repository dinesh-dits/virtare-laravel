...
class CreatePatientDevicesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('patientDevices', function (Blueprint $table) {
            $table->id();
            $table->string('udid');
            $table->bigInteger('patientId')->unsigned();
            $table->foreign('patientId')->references('id')->on('patients')->onDelete('cascade')->onUpdate('cascade');
            $table->bigInteger('otherDeviceId')->unsigned();
            $table->foreign('otherDeviceId')->references('id')->on('globalCodes')->onDelete('cascade')->onUpdate('cascade');
