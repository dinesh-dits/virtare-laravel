...
class CreateGeneralParametersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('generalParameters', function (Blueprint $table) {
            $table->id();
            $table->string('udid');
            $table->bigInteger('generalParameterGroupId')->unsigned();
            $table->foreign('generalParameterGroupId')->references('id')->on('generalParameterGroups')->onDelete('cascade')->onUpdate('cascade');
            $table->string('vitalFieldId');
            $table->integer('highLimit');
