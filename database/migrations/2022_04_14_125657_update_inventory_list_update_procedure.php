...
class UpdateInventoryListUpdateProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $procedure =
            "CREATE PROCEDURE `inventoryList`(IN isAvailableX TINYINT,IN deviceTypeX INT,IN active INT,IN search VARCHAR(100))
       BEGIN
        SELECT
        inventories.*,
        ...
        WHERE (inventories.isAvailable = isAvailableX OR isAvailableX='')
        AND (inventories.isActive=active OR active='')
        AND (deviceModels.deviceTypeId = deviceTypeX OR deviceTypeX='')
        AND ((globalCodes.name LIKE CONCAT('%',search,'%'))
        OR (deviceModels.modelName LIKE CONCAT('%',search,'%')))
        AND inventories.deletedAt IS NULL
        ORDER BY inventories.serialNumber ASC;
        END;";
        DB::unprepared($procedure);
    }
