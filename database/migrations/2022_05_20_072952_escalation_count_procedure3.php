...
class EscalationCountProcedure3 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $procedure = "
        CREATE PROCEDURE  escalationCount(IN idx int,IN fromDate VARCHAR(50),IN toDate VARCHAR(50)) 
        BEGIN
        SELECT(COUNT(escalationTypes.escalationTypesId)) AS total,
        globalCodes.name AS text,
        globalCodes.id AS id,
        hour(escalations.dueBy) as time
        FROM
        escalationTypes
        LEFT JOIN escalations.dueBy
        ...
        ON escalationStaff.escalationStaffId = escalations.escalationId
