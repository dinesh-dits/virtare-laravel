...
class GetPatientConditionsCountDailyProcedure6 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        WHERE
        (globalCodes.name='Patient' OR globalCodes.name='Both') AND patientFlags.deletedAt IS NULL AND patients.deletedAt IS NULL AND (patientFlags.createdAt BETWEEN fromDate AND toDate)
        GROUP BY
        flagId,text,color;
        END;";
