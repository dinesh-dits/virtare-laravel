...
class NotesListByPatientIdProcedure5 extends Migration
{
    /**
     * Run the migrations.
     *
...
LEFT JOIN patients as p1 ON p1.userId = notes.createdBy
LEFT JOIN users as u2 ON u2.id=p1.userId
WHERE notes.referenceId IN (SELECT id FROM patientFlags WHERE `patientID` = referenceId) AND notes.entityType = 'patientFlag'
UNION
