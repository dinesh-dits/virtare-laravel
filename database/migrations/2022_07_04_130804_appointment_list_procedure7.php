...
class AppointmentListProcedure7 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        WHERE  (appointments.statusId=statusId OR status="")  AND (startDateTime >= fromDate OR fromDate = "") AND (startDateTime <= toDate  OR toDate="") AND (staffId IN (SELECT * FROM JSON_TABLE( staffIdx, "$[*]" COLUMNS( Value INT PATH "$" ) ) as s) OR staffIdx = "" ) AND appointments.deletedAt IS NULL Group By appointments.id
