...
class AppointmentListProcedure8 extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $procedure =
            'CREATE PROCEDURE  appointmentList(fromDate VARCHAR(50),toDate VARCHAR(50),staffIdx VARCHAR(50),IN statusId INT)
        BEGIN
        ...
        WHERE  (appointments.statusId=statusId OR statusId="")  AND (startDateTime >= fromDate OR fromDate = "") AND (startDateTime <= toDate  OR toDate="") AND (staffId IN (SELECT * FROM JSON_TABLE( staffIdx, "$[*]" COLUMNS( Value INT PATH "$" ) ) as s) OR staffIdx = "" ) AND appointments.deletedAt IS NULL Group By appointments.id
