...
class NotesListByPatientIdAppointmentProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $procedure = "DROP PROCEDURE IF EXISTS `NotesListByPatientIdAppointment`";
        DB::unprepared($procedure);
        $procedure = "CREATE PROCEDURE `NotesListByPatientIdAppointment`(referenceId INT,fromDate VARCHAR(50),toDate VARCHAR(50))
           BEGIN 
...
WHERE notes.referenceId IN (SELECT id FROM appointments WHERE `patientID` = referenceId) AND notes.entityType = 'appointment' AND (notes.createdAt >= fromDate OR fromDate='') AND (notes.createdAt <= toDate OR toDate=''))
ORDER BY newId DESC;
END;";
        DB::unprepared($procedure);
    }
