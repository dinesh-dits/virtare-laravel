...
class NotesListByPatientIdPatientFlagProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $procedure = "DROP PROCEDURE IF EXISTS `NotesListByPatientIdPatientFlag`";
            DB::unprepared($procedure);

            $procedure = "CREATE PROCEDURE `NotesListByPatientIdPatientFlag`(referenceId INT,fromDate VARCHAR(50),toDate VARCHAR(50))
           BEGIN 
(SELECT notes.id as newId, notes.udid as udid, notes.createdAt as date, notes.note, 'General' as category, 'patientFlag' as type, notes.createdBy as userId,
...
WHERE notes.referenceId IN (SELECT id FROM patientFlags WHERE `patientID` = referenceId) AND notes.entityType = 'patientFlag' AND (notes.createdAt >= fromDate OR fromDate='') AND (notes.createdAt <= toDate OR toDate=''))
ORDER BY newId DESC;
