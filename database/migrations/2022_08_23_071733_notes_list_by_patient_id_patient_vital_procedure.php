...
class NotesListByPatientIdPatientVitalProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        $procedure = "DROP PROCEDURE IF EXISTS `NotesListByPatientIdPatientVital`";
            DB::unprepared($procedure);

            $procedure = "CREATE PROCEDURE `NotesListByPatientIdPatientVital`(referenceId INT,fromDate VARCHAR(50),toDate VARCHAR(50))
           BEGIN 
...
WHERE notes.referenceId IN (SELECT id FROM patientVitals WHERE `patientID` = referenceId) AND notes.entityType = 'patientVital' AND (notes.createdAt >= fromDate OR fromDate='') AND (notes.createdAt <= toDate OR toDate=''))
ORDER BY newId DESC;
