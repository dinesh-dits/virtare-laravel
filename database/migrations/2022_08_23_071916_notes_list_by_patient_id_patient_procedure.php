...
class NotesListByPatientIdPatientProcedure extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $procedure = "DROP PROCEDURE IF EXISTS `NotesListByPatientIdPatient`";
            DB::unprepared($procedure);

            $procedure = "CREATE PROCEDURE `NotesListByPatientIdPatient`(referenceId INT,fromDate VARCHAR(50),toDate VARCHAR(50))
           BEGIN 
...
WHERE notes.referenceId = referenceId AND notes.entityType = 'patient' AND (notes.createdAt >= fromDate OR fromDate='') AND (notes.createdAt <= toDate OR toDate=''))
ORDER BY newId DESC;
